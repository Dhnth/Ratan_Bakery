...
export default function AdminDashboard() {
    ...
    const loadData = async () => {
      ...
      try {
        const [statsRes, ordersRes, lowStockRes] = await Promise.all([
          ...
          fetch(
            `/api/admin/orders?search=${encodeURIComponent(debouncedSearch)}`,
          ),
  ...
  const statCards: StatCard[] = [
    ...
    },
    {
      ...
      change: stats?.pendingPayments ? "+2" : "Aman",
      trend: stats?.pendingPayments ? "down" : "stable",
      icon: Clock,
      color: "bg-red-50",
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h2 className="font-serif text-3xl md:text-4xl text-[#823b18] mb-1">
            Ringkasan Harian
          </h2>
          <p className="text-[#54433c] text-sm">
            Selamat datang, {session?.user?.name || "Kepala Baker"}. Ini kondisi
            dapur Anda hari ini.
          </p>
        </div>
        <motion.button
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                ...
                <div
                  className={`flex items-center gap-1 text-xs font-semibold ${getTrendColor(stat.trend)}`}
                >
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-[#dac1b8]/10 overflow-hidden">
            <div className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#dac1b8]/10">
              <h3 className="font-serif text-xl md:text-2xl text-[#28180b]">
                Pesanan Terbaru
              </h3>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-[#fff1e9]">
                  <tr>
                    <th className="px-5 py-3 text-xs font-semibold text-[#54433c] uppercase tracking-wider">
                      No. Pesanan
                    </th>
                    <th className="px-5 py-3 text-xs font-semibold text-[#54433c] uppercase tracking-wider">
                      Pelanggan
                ...
                <tbody className="divide-y divide-[#dac1b8]/5">
                  {orders.length === 0 ? (
                    <tr>
                      <td
                        colSpan={5}
                        className="px-5 py-12 text-center text-[#54433c]"
                      >
                      ...
                      return (
                        ...
                        >
                          <td className="px-5 py-4">
                            <span className="text-sm font-medium text-[#823b18]">
                              {order.orderNumber}
                            </span>
                          </td>
                          <td className="px-5 py-4 text-sm text-[#28180b]">
                            {order.customerName}
                          </td>
                          <td className="px-5 py-4 text-sm font-semibold text-[#28180b]">
          ...
          <div className="relative rounded-2xl overflow-hidden h-48 bg-gradient-to-r from-[#823b18] to-[#a0522d]">
            ...
            <div className="relative z-10 p-6 flex flex-col justify-center h-full">
              <h4 className="font-serif text-2xl text-white mb-2">
                Jadwal Adonan Harian
              </h4>
              <p className="text-white/80 max-w-xs text-sm mb-4">
                3 resep musiman baru siap untuk uji coba pemanggangan besok
                pagi.
              </p>
                ...
                lowStock.map((item, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-sm text-[#28180b]">
                          {item.name}
                        </p>
          ...
          <div className="bg-gradient-to-br from-[#ffeadc] to-[#ffe3cf] p-6 rounded-2xl shadow-sm relative overflow-hidden">
            <div className="relative z-10">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/30 backdrop-blur-sm rounded-full mb-3">
                ...
                <span className="text-[10px] font-semibold text-[#823b18] uppercase tracking-wider">
                  Aksi Cepat
                </span>
              </div>
              <h3 className="font-serif text-xl text-[#28180b] mb-4">
                Kirim Pengantaran Pagi
              </h3>
